...
public class HBoxSlip extends HBox {
    ...
    public HBoxSlip(MembershipListDTO m, Tab tt) {
        ...
        tt.selectedProperty().addListener((ObservableValue<? extends Boolean> observable, Boolean focusLost, Boolean focusGained) -> {
        ...
        slipWaitCheckBox.selectedProperty().addListener((observable, oldValue, newValue) -> SqlUpdate.updateWaitList(membership.getMsid(), "SLIPWAIT", newValue));

        kayakWaitCheckBox.selectedProperty().addListener((observable, oldValue, newValue) -> SqlUpdate.updateWaitList(membership.getMsid(), "KAYAKRACKWAIT", newValue));
        shedWaitCheckBox.selectedProperty().addListener((observable, oldValue, newValue) -> SqlUpdate.updateWaitList(membership.getMsid(), "SHEDWAIT", newValue));
        wantsToSubleaseCheckBox.selectedProperty().addListener((observable, oldValue, newValue) -> SqlUpdate.updateWaitList(membership.getMsid(), "WANTSUBLEASE", newValue));
        wantsToReleaseCheckBox.selectedProperty().addListener((observable, oldValue, newValue) -> SqlUpdate.updateWaitList(membership.getMsid(), "WANTRELEASE", newValue));
        slipChangeCheckBox.selectedProperty().addListener((observable, oldValue, newValue) -> SqlUpdate.updateWaitList(membership.getMsid(), "WANTSLIPCHANGE", newValue));
    ...
    private boolean isSubleasingSlip() {
        return SqlExists.slipRentExists(membership.getMsid());
    }
