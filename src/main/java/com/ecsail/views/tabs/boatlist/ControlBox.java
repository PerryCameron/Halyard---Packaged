...
public class ControlBox extends VBox {
    ...
    private VBox setUpFieldSelectedToSearchBox() {
        ...
        for(DbBoatSettingsDTO dto: parent.getModel().getBoatSettings()) {
    ...
    private String getLabel(String value) {
        return parent.getModel().getBoatSettings().stream().filter(setting -> setting.getFieldName().equals(value))
    ...
    private void fillTableView(String searchTerm) {
        if(!searchTerm.equals("")) {
            parent.getModel().getSearchedBoats().clear();
            parent.getModel().getSearchedBoats().addAll(searchString(searchTerm));
            parent.boatListTableView.setItems(parent.getModel().getSearchedBoats());
            parent.isActiveSearch = true;
        } else { // if search box has been cleared
            parent.boatListTableView.setItems(parent.getModel().getBoats());
    ...
    private ObservableList<BoatListDTO> searchString(String searchTerm) {
        ...
        boolean hasMatch = false;
            for(BoatListDTO boatListDTO: parent.getModel().getBoats()) {
    ...
    private HBox setUpRecordCountBox() {
        ...
        parent.numberOfRecords = new Text(String.valueOf(parent.getModel().getBoats().size()));
    ...
    private void updateRecordCount() {
        ...
        if(parent.isActiveSearch)
            number = parent.getModel().getSearchedBoats().size() + "/" + parent.getModel().getBoats().size();
        else
            number = String.valueOf(parent.getModel().getBoats().size());
    ...
    private VBox createRadioBox() {
        ...
        for(BoatListRadioDTO radio: parent.getModel().getBoatListRadioDTOs()) {
    ...
    protected void makeListByRadioButtonChoice()  {
        parent.getModel().getBoats().clear();
        Method method;
        try {
            method = parent.getModel().getBoatRepository().getClass().getMethod(parent.selectedRadioBox.getMethod());
            parent.getModel().getBoats().setAll((List<BoatListDTO>) method.invoke(parent.getModel().getBoatRepository()));
        } catch (NoSuchMethodException | IllegalAccessException | InvocationTargetException e) {
            throw new RuntimeException(e);
        }
        if(!parent.textField.getText().equals("")) fillTableView(parent.textField.getText());
        updateRecordCount();
        parent.getModel().getBoats().sort(Comparator.comparing(BoatListDTO::getBoatId));
    }
}
